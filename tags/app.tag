<app>
   ...
   <header title="Riot Chat App" ></header>
   ...
   var self = this;
      ...
      firebase_auth_obj.onAuthStateChanged(riotChatUser =>{
         self.app_loading = true;
         if(riotChatUser){
            if(riotChatUser.emailVerified == true && this.checkReverificationLink == false){
               // console.log("user logged in.");
               self.current_user.displayName = riotChatUser.displayName;
               self.current_user.email = riotChatUser.email;
               self.current_user.uid = riotChatUser.uid;
               self.userLoggedIn = true;
               self.newUser = false;
               self.signInHelp = false;
               var user_additional_data = user_ref.child(riotChatUser.uid);
